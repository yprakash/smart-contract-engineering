...
def handle_event(event):
    log = watcher_contract.events.UpkeepPerformed().processLog(event)
    logSender = log['args']['by']
    eventSig = log['args']['eventSig']
    upkeepNumber = log['args']['upkeepNumber']
    print(f"[NEW UPKEEP] #{upkeepNumber} by {logSender} | EventSig: {eventSig.hex()} | Block: {event['blockNumber']}")
